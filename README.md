...
현재 구현 상태와 남은 수동 확인은 [현재 작업 기준](PROJECT_MEMORY.md), 최신 변경은 [2026-08-26 유지보수 업데이트](docs/RELEASE-NOTES-2026-08-26.md)에서 확인합니다.
...
- 두 기준의 1위가 같은 물리 계획이면 과거 저장본도 `통합 추천안` 한 개로 표시합니다.
- 법적 상한 참고안은 산술 FAR을 그대로 쌓은 모형이 아닙니다. 법규 외곽선·배치·층간 연결을 통과한 상위 실현 FAR 후보 중 층판이 안정적인 3D 비교안을 표시합니다.
- 사용자는 추천안을 자동 적용하지 않고, 복제한 뒤 빠른 계획 또는 정밀 편집에서 조정합니다.
...
자동 검증이 통과해도 DAE/DXF를 실제 프로그램에서 여는 작업, 새 브라우저 전체 흐름, 보고서 A4 인쇄는 [릴리스 스모크 테스트](docs/release-smoke-test.md)에 따라 별도로 확인합니다.
