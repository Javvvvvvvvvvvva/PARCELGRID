...
코드 기준선은 다시 검증 가능한 상태다. 프로덕션 의존성 취약점, 깨진 데모 링크, 외부 조회 실패의 오표시, 건축물대장 미조회/빈 토지 혼동, 모바일 핵심 화면 넘침을 이번 배치에서 수정했다.
...
| 순서 | 항목 | 완료 기준 | 필요한 입력 |
| --- | --- | --- | --- |
| 1 | 실제 키 기반 골든 패스 | 새 브라우저에서 쌍문동 281-23 주소 검색부터 Stage 5 보고서까지 완주하고 모든 외부 응답의 출처·기준일 보존 | Kakao, VWorld, MOLIT 운영 키 |
| 2 | 실제 흑자 회귀 사례 | 검증된 주소 1건을 고정하고 Low/Base/High 손익, IRR, 최대 인수가를 원장과 대사 | 공개 가능한 주소와 검증 기준 |
| 3 | 내보내기 실물 QA | A4 PDF에 잘림이 없고 DAE/GLB/DXF를 실제 SketchUp·CAD에서 열어 Geometry Hash와 단위를 대조 | 대상 앱이 설치된 검수 환경 |
...
- Vitest 84 files / 331 tests 통과
- ESLint 및 TypeScript 통과
- 프로덕션 의존성 취약점 0건
- 390x844 status/comps 및 1440x900 status 브라우저 확인
- 보안 헤더와 역세권 503 응답 확인
