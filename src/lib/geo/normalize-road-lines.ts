...
/**
 * VWorld LT_L_SPRD can return MultiLineString geometry. Older lookup data
 * flattened every part into one point array, which creates a fake diagonal
 * segment between the end of one part and the start of the next.
 *
 * This function keeps continuous road segments intact and splits only large
 * outlier jumps relative to the road's normal vertex spacing. It is safe to
 * run for both new correctly separated data and legacy flattened data.
 */
