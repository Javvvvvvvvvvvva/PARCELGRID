...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (lawdCd) {
      ...
      const fetchType = (type: "land" | "officetel" | "commercial" | "apartment") =>
        fetchMolitRange({
          lawdCd,
          type,
      ...
      const [land, officetel, commercial, apartment] = await Promise.all([
        fetchType("land"),
        fetchType("officetel"),
      ...
      transactions = [...land, ...officetel, ...commercial, ...apartment];
      console.log(
        `MOLIT 거래 ${transactions.length}건 (토지 ${land.length} / 오피스텔 ${officetel.length} / 상업 ${commercial.length} / 아파트 ${apartment.length})`
      );
    }
