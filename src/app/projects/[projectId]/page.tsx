...
export default function DashboardPage({
  ...
  const draftAcquisitionPrices = useProjectStore(
    (state) => state.draftAcquisitionPrices,
  );
  const setDraftAcquisitionPrice = useProjectStore(
    (state) => state.setDraftAcquisitionPrice,
  );
  const resetDraftAcquisitionPrice = useProjectStore(
    (state) => state.resetDraftAcquisitionPrice,
  );
  ...
  const calculation = useMemo(() => {
    ...
    const maxAcquisition = findMaxAcquisitionForScenario(parcel, scenario, [
  ...
  if (!context.ready || !calculation) {
    const blocked = context.ready
      ? {
          title: "사업성 계산이 준비되지 않았습니다",
          message: "대표 계획안 계산을 다시 실행하세요.",
          code: "missing-finance-scenario",
        }
      : context;
    return (
      <Stage3Blocked
  ...
  const editedCount =
    Object.keys(draft ?? {}).length + (acquisitionEdited ? 1 : 0);
  const marketEstimate =
    data.parcel.acquisitionEstimate?.estimatedPriceManwon ??
  ...
  const marketMedianTotal =
    data.parcel.acquisitionEstimate?.marketMedianManwon ?? 0;
  const houseProxy = data.parcel.acquisitionEstimate?.houseProxy;
  const acquisitionPerPyeong = acquisitionPricePerPyeong(
    acquisitionPrice,
    data.parcel.lotArea,
  );
  const publicValueTotal = Math.round(
    (data.parcel.landPrice * data.parcel.lotArea) / 10_000,
  );
  ...
  const profitTone: Tone =
    result.profit > 0 ? "positive" : result.profit < 0 ? "negative" : "review";
  ...
  const visibleRescue = rescueRun?.key === rescueKey ? rescueRun.result : null;
  const visibleStress = stressRun?.key === rescueKey ? stressRun.result : null;
    ...
    <div className="stage3-page">
      <header className="stage3-header">
        <div>
          ...
          <p>
            Stage 2 대표 매스에 관측자료와 검토 가정을 연결한 예비 사업성
            모델입니다. 금융기관 약정·시공사 견적·세무 검토 전에는 투자
            확정값으로 사용할 수 없습니다.
          </p>
        </div>
      ...
      <section className="audit-banner" role="status">
        ...
        <span>
          실거래 원본, 사용자 입력, 알고리즘 추정과 미산정 항목을 분리합니다.
          현재 결과는 비교·민감도 검토용이며 매입가 확정, 대출 심사 또는
          세무신고용이 아닙니다.
        </span>
      </section>
      ...
      <section className="source-lock">
        <div className="source-main">
          ...
          <span>
            v{planningScenario.version} · 지상 {aboveFloors}층
            {basementFloors ? ` / 지하 ${basementFloors}층` : ""} · 프로그램{" "}
            {geometry.building.totalProgramAreaSqm.toFixed(1)}㎡
          </span>
        </div>
        <Fact
          label="실현 용적률"
        ...
        <Fact
          label="금융 원장"
          value={`${PROJECT_LEDGER_MODEL_VERSION} · ${data.meta.version}`}
          mono
        />
      </section>
      ...
      <section className="decision-grid">
        ...
        <DecisionSignal
          ...
          value={
            dscrApplicable ? `DSCR ${result.dscr.toFixed(2)}` : "임대 NOI 없음"
          }
          tone={
            dscrApplicable
              ? result.dscr >= 1.3
      ...
      <section className="dashboard-section land-section" id="land-price">
        ...
        <div className="method-strip">
          ...
          <span>
            토지 분포 표본{" "}
            <b>
              {data.parcel.acquisitionEstimate?.transactionCount ?? "미보존"}건
            </b>
          </span>
          <span>
            동일 지목 표본{" "}
            <b>
              {data.parcel.acquisitionEstimate?.details?.sampleSize ?? "미보존"}
            </b>
          </span>
          <span>
            입지 티어{" "}
            <b>
              {data.parcel.acquisitionEstimate?.details?.locationTier ??
                "이전 프로젝트"}
            </b>
          </span>
          <button
            className="text-button"
            onClick={() => router.push(`/projects/${projectId}/comps`)}
          >
            실거래 근거 열기
          </button>
        </div>
      ...
      <div className="workspace-grid">
        <main className="stage3-main">
          <section className="dashboard-section">
            <SectionHeader
              eyebrow="PRELIMINARY PRO FORMA"
              title="예비 사업수지"
              description="공사비·금융·매출 가정을 한 원장에서 계산한 세전 결과입니다. 세금은 계산 가능한 항목만 별도 표시하며 미산정 항목이 있습니다."
            />
            <div className="metric-grid">
              <MetricCard
                label="세전 총사업비"
                value={won(result.totalCost)}
                note="세금·미확정 부대비 일부 별도"
              />
              <MetricCard
                label="가정상 매출·가치"
                value={won(result.totalRevenue)}
              />
              <MetricCard
                label="세전 예상손익"
                value={won(result.profit)}
              ...
              <MetricCard
                label="예비 PF 최고잔액"
                value={won(result.pfLoan)}
                note={`실제 LTC ${result.ltc.toFixed(1)}%`}
              />
              <MetricCard
                label="세전 IRR"
                value={irrAvailable ? `${result.irr.toFixed(1)}%` : "N/A"}
                note={irrAvailable ? undefined : "유일한 IRR을 산정할 수 없음"}
                tone={irrTone}
              />
              <MetricCard
                label="세전 NPV"
                value={won(result.npv)}
                note={`할인율 ${assumptions.equityIRR.toFixed(1)}%`}
            ...
            <div className="return-strip">
              ...
              <ReturnMetric
                label="최대 자금노출"
                value={won(Math.abs(result.maxExposure))}
              />
              <ReturnMetric
                label="회수 예상"
                value={
              ...
              <ReturnMetric
                label="세금 부분 추정액"
                value={won(taxes.total)}
                note={taxes.complete ? "계산 완료" : "재산세·부가세 등 미산정"}
              />
            </div>
          </section>
          ...
          <section className="dashboard-section split-section">
            <BreakdownPanel
              ...
              items={[
                ["토지비", result.landCost],
                ["철거비", result.demolitionCost],
                ["직접 공사비", result.hardCost],
                ["설계·인허가·간접비", result.softCost],
                ["금융비", result.financingCost],
                ["예비비", result.contingency],
              ]}
            />
            <BreakdownPanel
              title="매출·가치 구성"
              ...
              items={[
                ["분양·통매각", result.revenueSale],
                ["임대 가치", result.revenueLease],
                ["근생 가치", result.revenueRetail],
              ]}
            />
          </section>
          ...
          <section className="dashboard-section" id="sensitivity">
            ...
            <div className="break-even-strip">
              <div>
                ...
                <strong>
                  {formatRevenuePrice(breakEven.current, breakEven.kind)}
                </strong>
              </div>
              <div>
                ...
                <strong>
                  {breakEven.value == null
                    ? "계산 범위 밖"
                    : formatRevenuePrice(breakEven.value, breakEven.kind)}
                </strong>
              </div>
              <div>
                <span>가격 안전마진</span>
                <strong>
                  {breakEven.marginPct == null
                    ? "—"
                    : `${breakEven.marginPct >= 0 ? "+" : ""}${breakEven.marginPct.toFixed(1)}%`}
                </strong>
              </div>
            </div>
          </section>
          ...
          <section className="dashboard-section" id="cashflow">
            <SectionHeader
              eyebrow="UNIFIED CASH FLOW"
              title="분기 자금흐름"
              description="IRR·NPV·PF 이자와 동일한 월별 원장을 분기로 합산합니다. 설계·공사·회수 기간 입력과 공사비 100%가 반영됩니다."
            />
          ...
          <section className="dashboard-section" id="price-evidence">
            <SectionHeader
              eyebrow="EXIT PRICE EVIDENCE"
              title="매각·분양 가격 근거"
              description="가격 추정치와 사용자가 수정한 계산 가정을 구분합니다. 사례가 부족하면 숫자를 확정값으로 표시하지 않습니다."
            />
            <div className="evidence-summary">
              <EvidenceMetric
                label="현재 적용 단가"
                value={`${Math.round((assumptions.salePricePerSqM * SQM_PER_PYEONG) / 10_000).toLocaleString()}만원/평`}
              />
              <EvidenceMetric
                label="채택 사례 중앙값"
                value={
                  data.saleEstimate
                    ? `${data.saleEstimate.medianPPP.toLocaleString()}만원/평`
                    : "근거 부족"
                }
              />
              <EvidenceMetric
                ...
                value={
                  data.saleEstimate
                    ? `${data.saleEstimate.conservativePPP.toLocaleString()}만원/평`
                    : "근거 부족"
                }
              />
              <EvidenceMetric
                label="표본 근거 수준"
                value={confidenceLabel(saleConfidence)}
              />
            </div>
            <div className="case-list">
              {(data.saleEstimate?.cases ?? []).slice(0, 5).map((item) => (
                <div
                  className="case-row"
                  key={`${item.address}-${item.date}-${item.pricePerPyeong}`}
                >
                  <span>
                    <b>{item.address}</b>
                    <small>
              ...
              {!data.saleEstimate && (
                <div className="empty-evidence">
                  채택 가능한 단독·다가구 유사 사례가 3건 미만입니다. 현재
                  가격은 시장 확정값이 아니라 기본 가정입니다.
                </div>
              )}
            </div>
            <button
          ...
          {data.scenarios.length > 1 && (
            <section className="dashboard-section" id="plan-comparison">
              <SectionHeader
                ...
                title="저장 계획안 사업성 비교"
                description="Stage 2에서 저장한 계획안만 비교하며, 현재 대표안과 동일한 기본 시장 가정을 사용합니다."
              />
              <ScenarioTable
        ...
        <aside className="stage3-sidebar">
          <section className="assumption-panel" id="inputs">
            <div className="assumption-heading">
              <div>
                <span className="section-kicker">LIVE INPUTS</span>
                <h2>계산 조정</h2>
              </div>
              {editedCount > 0 && (
              ...
              <details key={group.title} open className="assumption-group">
                ...
                {group.fields.map((field) => (
                  <NumericEditor
                    ...
                    source={
                      ASSUMPTION_META[field.field]?.basis ?? "사용자 확인 필요"
                    }
                    onChange={(value) =>
                      setDraftAssumption(financeScenario.id, field.field, value)
                    }
                  />
                ))}
              </details>
          ...
          <section className="evidence-panel">
            ...
            <EvidenceRow
              label="토지 참고 추정"
              value={`${data.parcel.acquisitionEstimate?.modelVersion ?? "legacy-unversioned"} · 자체 보정·외부 검증 전`}
              state="미확정"
            />
            <EvidenceRow
              label="매각 단가"
              value={
                data.saleEstimate
                  ? `${data.saleEstimate.modelVersion} · ${data.saleEstimate.basis}`
                  : "내부 초기 가정 · 실거래 근거 없음"
              }
              state="미확정"
            />
            <EvidenceRow
              label="공사비"
              value={ASSUMPTION_META.constCostPerSqM?.kind ?? "가정값"}
              state="미확정"
            />
            {hasLeaseRevenue && (
              <>
                <EvidenceRow
                  label="임대 출구 보정"
                  value={`${(INTERNAL_REVENUE_ASSUMPTIONS.stabilizationDiscount * 100).toFixed(0)}% 자체 가정 · 외부 검증 전`}
                  state="미확정"
                />
                <EvidenceRow
                  label="근생 임대료 배수"
                  value={`주거 임대료의 ${INTERNAL_REVENUE_ASSUMPTIONS.retailRentPremium.toFixed(1)}× 자체 가정`}
                  state="미확정"
                />
              </>
            )}
            <EvidenceRow
              label="PF 조건"
              value="사용자·금융기관 확인 필요"
              state="미확정"
            />
            <EvidenceRow
              label="세금"
              value={`${TAX_MODEL_AS_OF} 기본세율 · 미산정 항목 있음`}
              state="미확정"
            />
            <p className="coverage-note">
              현재 대표 계획의 손익에 직접 연결되는 입력만 표시합니다. 임대
              안정화 개월과 월 분양속도처럼 아직 핵심 손익 엔진에 연결되지 않은
              값도 편집 항목에서 제외했습니다.
            </p>
          </section>
          ...
          <section className="next-actions">
            ...
            <button
              ...
              onClick={() => router.push(`/projects/${projectId}/handoff`)}
            >
              전문가 검증·인계
            </button>
          </section>
      ...
      <style jsx>{`
        ...
        .return-strip > :global(div) + :global(div) {
            ...
            return (
              <article
                ...
                style={{
                  ...
                  borderRadius: 9,
                  background: reachable ? "var(--pos-soft)" : "var(--bg-soft)",
                }}
              >
                <div
                  style={{
                ...
                >
                  ...
                  <small
                    style={{
                      color: reachable ? "var(--pos-fg)" : "var(--fg-muted)",
                      fontWeight: 800,
                    }}
                  >
                    {reachable
...
function formatRevenuePrice(value: number, kind: "sale" | "rent") {
  if (kind === "sale")
    return `${Math.round((value * SQM_PER_PYEONG) / 10_000).toLocaleString()}만원/평`;
  ...
  return (
    <div style={{ maxWidth: 840, margin: "0 auto", padding: "72px 28px" }}>
      ...
      >
        ...
        >
          구형 권장안이나 다른 계획안의 숫자로 대체하지 않았습니다.
          대표안·Geometry Snapshot·금융 시나리오가 정확히 일치할 때만
          표시합니다.
        </p>
        <button
          onClick={() => router.push(`/projects/${projectId}/envelope`)}
...
function Fact({
  ...
  return (
    ...
    >
      <span style={{ fontSize: 9.5, color: "var(--fg-muted)" }}>{label}</span>
      <strong
        style={{
          fontSize: mono ? 10 : 13,
...
function DecisionSignal({
  ...
  return (
    <article
      style={{
        border: "1px solid var(--border)",
        borderTop: `3px solid ${colors.edge}`,
        borderRadius: 10,
        padding: "12px 13px",
        background: "var(--bg-elev)",
      }}
...
function SectionHeader({
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize: 17, margin: "6px 0 4px" }}>{title}</h2>
      <p
        style={{
          margin: 0,
          color: "var(--fg-muted)",
...
function PricePoint({
  ...
  value: number;
  note: string;
  tone?: "default" | "market" | "active" | "safe" | "risk";
}) {
  const edge =
    tone === "active"
      ? "var(--accent)"
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 9,
        background: tone === "active" ? "var(--accent-soft)" : "var(--bg-soft)",
      }}
    >
      <span style={{ fontSize: 9.5, color: "var(--fg-muted)" }}>{label}</span>
      <strong
...
function ReturnMetric({
  ...
  return (
    <div style={{ padding: "10px 11px", background: "var(--bg-elev)" }}>
      <span
        style={{ display: "block", fontSize: 8.5, color: "var(--fg-muted)" }}
      >
        {label}
      </span>
      <strong
      ...
      {note && (
        <small style={{ fontSize: 8, color: "var(--fg-faint)" }}>{note}</small>
      )}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
        <strong style={{ fontSize: 13 }}>{title}</strong>
        <strong style={{ fontSize: 11, fontFamily: "var(--font-mono)" }}>
          {won(total)}
        </strong>
      </div>
  ...
  return (
    <div style={{ overflowX: "auto", marginTop: 14 }}>
      ...
      >
        <thead>
          <tr>
            <th style={{ textAlign: "left", color: "var(--fg-muted)" }}>
              공사비 \ 가격
            </th>
            {sensitivity.revenueSteps.map((step) => (
              <th key={step} style={{ minWidth: 84, color: "var(--fg-muted)" }}>
                {step > 0 ? "+" : ""}
                {step}%
              </th>
            ))}
        ...
        <tbody>
          {sensitivity.rows.map((row, ri) => (
            <tr key={sensitivity.costSteps[ri]}>
              <th style={{ textAlign: "left", color: "var(--fg-muted)" }}>
                {sensitivity.costSteps[ri] > 0 ? "+" : ""}
                {sensitivity.costSteps[ri]}%
              </th>
              {row.map((cell, ci) => {
                const base =
                  sensitivity.costSteps[ri] === 0 &&
                  sensitivity.revenueSteps[ci] === 0;
                const tone: Tone = cell.profit >= 0 ? "positive" : "negative";
                const colors = toneColors(tone);
                return (
                  <td
                    key={sensitivity.revenueSteps[ci]}
                    style={{
                      padding: "9px 7px",
                      textAlign: "center",
                      borderRadius: 7,
                      outline: base ? "2px solid var(--fg)" : "none",
                      background: colors.bg,
                      color: colors.fg,
                    }}
                  >
  ...
  return (
    <div style={{ overflowX: "auto", marginTop: 13 }}>
      ...
      >
        <thead>
          <tr style={{ color: "var(--fg-muted)" }}>
            <th style={{ textAlign: "left", padding: 8 }}>분기</th>
            <th style={{ textAlign: "left", padding: 8 }}>단계</th>
            <th style={{ textAlign: "right", padding: 8 }}>유입</th>
            <th style={{ textAlign: "right", padding: 8 }}>유출</th>
            <th style={{ textAlign: "right", padding: 8 }}>누적</th>
          </tr>
        </thead>
        <tbody>
          {visible.map((row) => (
            ...
            >
              <td style={{ padding: 8 }}>{row.quarter}</td>
              <td style={{ padding: 8 }}>{row.phase}</td>
              <td
                style={{
                  padding: 8,
                  textAlign: "right",
              ...
              <td
                style={{
                  ...
                  textAlign: "right",
                  fontFamily: "var(--font-mono)",
                  color: row.cumulative < 0 ? "var(--neg-fg)" : "var(--pos-fg)",
                }}
              >
                {won(row.cumulative)}
              </td>
      ...
      {rows.length > visible.length && (
        <p style={{ margin: "8px 0 0", fontSize: 9, color: "var(--fg-muted)" }}>
          전체 {rows.length}개 분기 중 초기 12개 분기 표시
        </p>
      )}
    </div>
  );
...
function EvidenceMetric({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      <span style={{ display: "block", fontSize: 9, color: "var(--fg-muted)" }}>
        {label}
      </span>
      ...
      >
        <input
          ...
          onChange={(event) => {
            ...
            onChange(
              mode === "total" ? next : acquisitionPriceTotal(next, lotAreaSqm),
            );
          }}
          style={{
            width: 110,
        ...
        />
        <small
          style={{ minWidth: 54, color: "var(--fg-faint)", fontSize: 8.5 }}
        >
    ...
    <label style={{ display: "block", padding: "8px 0" }}>
      ...
      >
        ...
        <span style={{ display: "inline-flex", gap: 4, alignItems: "center" }}>
          ...
          />
          <small style={{ color: "var(--fg-faint)", fontSize: 8.5 }}>
            {suffix}
          </small>
        </span>
      </span>
...
function EvidenceRow({
  ...
  const tone: Tone =
    state === "확정" ? "positive" : state === "검토" ? "review" : "negative";
  return (
    <div
      style={{
        padding: "8px 0",
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
        <span style={{ fontSize: 9.5, color: "var(--fg-muted)" }}>{label}</span>
        <strong style={{ fontSize: 8.5, color: toneColors(tone).fg }}>
          {state}
        </strong>
      </div>
      <div
      ...
      >
        <thead>
          <tr style={{ color: "var(--fg-muted)", textAlign: "right" }}>
            <th style={{ textAlign: "left", padding: 8 }}>계획안</th>
            <th style={{ padding: 8 }}>총사업비</th>
            <th style={{ padding: 8 }}>손익</th>
            <th style={{ padding: 8 }}>IRR</th>
            <th style={{ padding: 8 }}>NPV</th>
          </tr>
        </thead>
        <tbody>
          {scenarios.map((scenario) => {
            ...
            return (
              ...
              >
                <td style={{ textAlign: "left", padding: 9 }}>
                  <strong>{scenario.name}</strong>
                  {representative && (
                    <small style={{ marginLeft: 6, color: "var(--accent)" }}>
                      대표안
                    </small>
                  )}
                </td>
                <td style={{ padding: 9, fontFamily: "var(--font-mono)" }}>
                  {won(scenario.cost)}
                </td>
                <td
                  style={{
                    ...
                    fontFamily: "var(--font-mono)",
                    color:
                      scenario.profit >= 0 ? "var(--pos-fg)" : "var(--neg-fg)",
                  }}
                >
                  {won(scenario.profit)}
                </td>
                <td style={{ padding: 9, fontFamily: "var(--font-mono)" }}>
                  {scenario.irrStatus === "calculated"
...
function toneColors(tone: Tone) {
  if (tone === "positive")
    return { bg: "var(--pos-soft)", fg: "var(--pos-fg)", edge: "var(--pos)" };
  if (tone === "negative")
    return {
      bg: "var(--neg-soft)",
      fg: "var(--neg-fg)",
