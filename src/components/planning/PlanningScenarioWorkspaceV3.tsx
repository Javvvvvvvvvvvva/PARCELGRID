...
function planHealth(calculation: PlanningScenarioCalculation): { fail: number; review: number } {
...
function Metric({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div style={{ padding: 12, border: "1px solid var(--border)", borderRadius: 10 }}>
      ...
      <div style={{ marginTop: 4, fontSize: 17, fontWeight: 750, letterSpacing: "-0.025em" }}>
        {value}
      </div>
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", gap: 9 }}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ marginTop: 5, fontSize: 10.5, color: "var(--fg-muted)" }}>
            {calculation.metrics.aboveGroundFloors}층
            {calculation.metrics.undergroundFloors > 0 ? ` · B${calculation.metrics.undergroundFloors}` : ""}
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 9, flexWrap: "wrap" }}>
        ...
        <span style={{ marginLeft: "auto", fontSize: 10, color: "var(--fg-faint)" }}>v{scenario.version}</span>
      </div>
    </article>
  );
}
...
function MiniMetric({
  ...
  return (
    <div style={{ padding: "7px 8px", background: "var(--bg-sunken)", borderRadius: 7 }}>
      ...
      <div style={{ marginTop: 2, fontSize: 10.5, fontWeight: 700, color }}>{value}</div>
    </div>
  );
}

export function PlanningScenarioWorkspaceV3({ projectId }: { projectId: string }) {
  ...
  const selectedPlanningScenarioId = useProjectStore((state) => state.selectedPlanningScenarioId);
  const addPlanningScenario = useProjectStore((state) => state.addPlanningScenario);
  const editPlanningScenarioDraft = useProjectStore((state) => state.editPlanningScenarioDraft);
  ...
  const duplicatePlanningScenario = useProjectStore((state) => state.duplicatePlanningScenario);
  ...
  useEffect(() => {
    ...
    addPlanningScenario(createStarterPlanningScenario(scenarioSeed(parcel), envelopePlan, projectId));
  ...
  return (
    <div style={{ maxWidth: 1380, margin: "0 auto", padding: "var(--s6) var(--s5) 80px" }}>
      <div style={{ display: "flex", gap: 8, marginBottom: "var(--s5)", fontSize: 11.5, color: "var(--fg-muted)" }}>
        <Link href={`/projects/${projectId}/status`} style={{ color: "inherit", textDecoration: "none" }}>
          Stage 1 현황 분석
        </Link>
        <span>→</span>
      ...
      <div className="parcel-metrics">
        ...
        <Metric label="저장 계획안" value={`${projectScenarios.length}개`} />
      </div>
          ...
          <div className="new-plan-grid">
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                <ModeButton active={newPlanMode === "blank"} onClick={() => setNewPlanMode("blank")} label="빈 계획" />
                <ModeButton
            ...
            <div style={{ display: "flex", alignItems: "end", justifyContent: "flex-end", gap: 7 }}>
              <button type="button" style={smallButtonStyle} onClick={() => setNewPlanOpen(false)}>취소</button>
              <button type="button" style={primaryButtonStyle} onClick={addPlan}>계획안 만들기</button>
            </div>
          </div>
        </Panel>
      )}
      ...
      <div className="workspace-grid">
        <aside>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 9 }}>
            ...
            <span style={{ fontSize: 10, color: "var(--fg-faint)" }}>클릭 즉시 전환</span>
          </div>
        ...
        <main>
          {selectedScenario && selectedCalculation ? (
            <div style={{ display: "grid", gap: "var(--s5)" }}>
              <Panel bodyStyle={{ padding: "var(--s5)" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}>
                      <input
                        ...
                        onChange={(event) => editPlanningScenarioDraft(selectedScenario.id, { name: event.target.value })}
                        aria-label="계획안 이름"
                        style={{ ...inputStyle, maxWidth: 360, fontSize: 18, fontWeight: 750 }}
                      />
                      <span className="ui-tag">{ORIGIN_LABEL[selectedScenario.origin]}</span>
                      <span className="ui-tag">v{selectedScenario.version}</span>
                      {selectedScenario.status === "draft" && <span className="ui-tag">미저장 변경</span>}
                    </div>
                    <p style={{ margin: "6px 0 0", fontSize: 10.5, color: "var(--fg-muted)" }}>
                      층별 편집 결과가 3D·법규·사업성에 실시간 반영됩니다.
                    </p>
                  </div>
                  <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                    <button type="button" style={smallButtonStyle} onClick={() => duplicatePlanningScenario(selectedScenario.id)}>복제</button>
                    <button type="button" style={{ ...smallButtonStyle, color: "var(--neg-fg)" }} onClick={deleteSelected}>삭제</button>
                    <button type="button" style={primaryButtonStyle} onClick={saveSelected}>계획안 저장</button>
                  </div>
                </div>
                ...
                <div className="selected-metrics">
                  ...
                  <Metric
                    ...
                    sub={selectedCalculation.metrics.parkingShortfallCars > 0
              ...
              <Panel title="개략 사업성" source="Stage 2 비교용 · Stage 3에서 정밀 계산" bodyStyle={{ padding: "var(--s5)" }}>
                <div className="economics-grid">
                  ...
                  <Metric
                    label="예상 이익"
                    value={won(selectedCalculation.economicsPreview.profitManwon, { full: true, sign: true })}
                    sub={`이익률 ${selectedCalculation.economicsPreview.profitMarginPct.toFixed(1)}%`}
                  />
                  <Metric label="연간 NOI" value={won(selectedCalculation.economicsPreview.expectedAnnualNoiManwon, { full: true })} />
                </div>
              </Panel>

              <div className="lower-grid">
                <Panel title="층별 프로그램 편집" source="실시간 재계산" bodyStyle={{ padding: "var(--s4)" }}>
                  <FloorProgramEditor
                    scenario={selectedScenario}
                    onChange={(patch) => editPlanningScenarioDraft(selectedScenario.id, patch)}
                  />
                </Panel>
                ...
                <Panel title="법규·데이터 점검" source="예비 판정" bodyStyle={{ padding: "var(--s4)" }}>
                  <div style={{ display: "grid", gap: 9 }}>
                    {selectedCalculation.checks.map((check) => (
                      <div key={check.code} style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 8 }}>
            ...
            <Panel bodyStyle={{ padding: 40 }}>
              <div style={{ textAlign: "center", color: "var(--fg-muted)" }}>계획안을 추가하세요.</div>
            </Panel>
          )}
        </main>
      </div>
